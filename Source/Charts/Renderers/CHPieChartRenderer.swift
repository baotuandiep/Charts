...
open class CHPieChartRenderer: CHDataRenderer
{
    @objc open weak var chart: CHPieChartView?

    @objc public init(chart: CHPieChartView, animator: CHAnimator, viewPortHandler: CHViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        if pieData != nil
        {
            ...
            for set in pieData!.dataSets as! [CHIPieChartDataSet]
            {
    ...
    @objc open func getSliceSpace(dataSet: CHIPieChartDataSet) -> CGFloat
    {
        guard
            dataSet.automaticallyDisableSliceSpacing,
            let data = chart?.data as? CHPieChartData
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: CHIPieChartDataSet)
    {
        guard let chart = chart else {return }
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        let yValueSum = (data as! CHPieChartData).yValueSum
        ...
        {
            guard let dataSet = dataSets[i] as? CHIPieChartDataSet else { continue }
            ...
            {
                guard let e = dataSet.entryForIndex(j) else { continue }
                let pe = e as? CHPieChartDataEntry
    ...
    open override func drawHighlighted(context: CGContext, indices: [CHHighlight])
    {
        ...
        for i in 0 ..< indices.count
        {
            ...
            guard let set = data.getDataSetByIndex(indices[i].dataSetIndex) as? CHIPieChartDataSet else { continue }
    ...
    private func createAccessibleElement(withIndex idx: Int,
                                         container: CHPieChartView,
                                         dataSet: CHIPieChartDataSet,
        ...
        guard let data = container.data as? CHPieChartData else { return element }
        ...
        let pieChartDataEntry = (dataSet.entryForIndex(idx) as? CHPieChartDataEntry)
