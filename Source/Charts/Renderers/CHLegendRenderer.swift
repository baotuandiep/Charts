...
open class CHLegendRenderer: CHRenderer
{
    /// the legend object this renderer renders
    @objc open var legend: CHLegend?

    @objc public init(viewPortHandler: CHViewPortHandler, legend: CHLegend?)
    {
    ...
    @objc open func computeLegend(data: CHChartData)
    {
        guard let legend = legend else { return }
        
        if !legend.isLegendCustom
        {
            var entries: [CHLegendEntry] = []
            ...
            for i in 0..<data.dataSetCount
            {
                ...
                if dataSet is CHIBarChartDataSet &&
                    (dataSet as! CHIBarChartDataSet).isStacked
                {
                    let bds = dataSet as! CHIBarChartDataSet
                ...
                else if dataSet is CHIPieChartDataSet
                {
                    let pds = dataSet as! CHIPieChartDataSet
                    
                    for j in 0..<min(clrs.count, entryCount)
                    {
                        entries.append(
                            CHLegendEntry(
                                label: (pds.entryForIndex(j) as? CHPieChartDataEntry)?.label,
                                form: dataSet.form,
                                formSize: dataSet.formSize,
                                formLineWidth: dataSet.formLineWidth,
                                formLineDashPhase: dataSet.formLineDashPhase,
                ...
                else if dataSet is CHICandleChartDataSet &&
                    (dataSet as! CHICandleChartDataSet).decreasingColor != nil
                {
                    let candleDataSet = dataSet as! CHICandleChartDataSet
    ...
    {
        CHChartUtils.drawText(context: context, text: label, point: CGPoint(x: x, y: y), align: .left, attributes: [NSAttributedStringKey.font: font, NSAttributedStringKey.foregroundColor: textColor])
    }
}
