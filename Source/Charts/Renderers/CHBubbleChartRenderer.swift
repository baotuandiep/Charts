...
open class CHBubbleChartRenderer: CHBarLineScatterCandleBubbleRenderer
{
    ...
    @objc open weak var dataProvider: CHBubbleChartDataProvider?
    
    @objc public init(dataProvider: CHBubbleChartDataProvider, animator: CHAnimator, viewPortHandler: CHViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        if let chart = dataProvider as? CHBubbleChartView {
        ...
        for (i, set) in (bubbleData.dataSets as! [CHIBubbleChartDataSet]).enumerated() where set.isVisible
        {
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: CHIBubbleChartDataSet, dataSetIndex: Int)
    {
        ...
        {
            guard let entry = dataSet.entryForIndex(j) as? CHBubbleChartDataEntry else { continue }
    ...
    open override func drawValues(context: CGContext)
    {
        guard let
            ...
            let dataSets = bubbleData.dataSets as? [CHIBubbleChartDataSet]
            else { return }
        ...
        for i in 0..<dataSets.count
        {
            ...
            {
                guard let e = dataSet.entryForIndex(j) as? CHBubbleChartDataEntry else { break }
    ...
    open override func drawHighlighted(context: CGContext, indices: [CHHighlight])
    {
        ...
        {
            guard
                let dataSet = bubbleData.getDataSetByIndex(high.dataSetIndex) as? CHIBubbleChartDataSet,
                dataSet.isHighlightEnabled,
                let entry = dataSet.entryForXValue(high.x, closestToY: high.y) as? CHBubbleChartDataEntry,
    ...
    {
        guard let chart = dataProvider as? CHBubbleChartView else { return [] }
    ...
    private func createAccessibleElement(withIndex idx: Int,
                                         container: CHBubbleChartView,
                                         dataSet: CHIBubbleChartDataSet,
                                         dataSetIndex: Int,
                                         shapeSize: CGFloat,
