...
open class CHBarChartRenderer: CHBarLineScatterCandleBubbleRenderer
{
    ...
    @objc open weak var dataProvider: CHBarChartDataProvider?
    
    @objc public init(dataProvider: CHBarChartDataProvider, animator: CHAnimator, viewPortHandler: CHViewPortHandler)
    {
    ...
    open override func initBuffers()
    {
        if let barData = dataProvider?.barData
        {
            ...
            {
                let set = barData.dataSets[i] as! CHIBarChartDataSet
    ...
    private func prepareBuffer(dataSet: CHIBarChartDataSet, index: Int)
    {
        guard
        ...
        for i in stride(from: 0, to: min(Int(ceil(Double(dataSet.entryCount) * animator.phaseX)), dataSet.entryCount), by: 1)
        {
            ...
            if !containsStacks || vals == nil
            {
                ...
                if let offsetView = dataProvider as? CHBarChartView {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        if let chart = dataProvider as? CHBarChartView {
        ...
        for i in 0 ..< barData.dataSetCount
        {
            ...
            if set.isVisible
            {
                if !(set is CHIBarChartDataSet)
                {
                    fatalError("Datasets for BarChartRenderer must conform to IBarChartDataset")
                }
                
                drawDataSet(context: context, dataSet: set as! CHIBarChartDataSet, index: i)
            }
        }
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: CHIBarChartDataSet, index: Int)
    {
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[dataSetIndex] as? CHIBarChartDataSet else { continue }
    ...
    {
        CHChartUtils.drawText(context: context, text: value, point: CGPoint(x: xPos, y: yPos), align: align, attributes: [NSAttributedStringKey.font: font, NSAttributedStringKey.foregroundColor: color])
    }
    ...
    open override func drawHighlighted(context: CGContext, indices: [CHHighlight])
    {
        ...
        {
            guard
                let set = barData.getDataSetByIndex(high.dataSetIndex) as? CHIBarChartDataSet,
    ...
    internal func setHighlightDrawPos(highlight high: CHHighlight, barRect: CGRect)
    {
    ...
    {
        guard let chart = dataProvider as? CHBarChartView else { return [] }
    ...
    internal func createAccessibleElement(withIndex idx: Int,
                                          container: CHBarChartView,
                                          dataSet: CHIBarChartDataSet,
                                          dataSetIndex: Int,
                                          stackSize: Int,
