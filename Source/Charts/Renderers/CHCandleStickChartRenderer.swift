...
open class CHCandleStickChartRenderer: CHLineScatterCandleRadarRenderer
{
    @objc open weak var dataProvider: CHCandleChartDataProvider?
    
    @objc public init(dataProvider: CHCandleChartDataProvider, animator: CHAnimator, viewPortHandler: CHViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        if let chart = dataProvider as? CHCandleStickChartView {
        ...
        for set in candleData.dataSets as! [CHICandleChartDataSet]
        {
            if set.isVisible
            {
    ...
    @objc open func drawDataSet(context: CGContext, dataSet: CHICandleChartDataSet)
    {
        guard
            let dataProvider = dataProvider,
            let chart = dataProvider as? CHCandleStickChartView
            else { return }
        ...
        {
            // get the entry
            guard let e = dataSet.entryForIndex(j) as? CHCandleChartDataEntry else { continue }
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[i] as? CHIBarLineScatterCandleBubbleChartDataSet
                    else { continue }
                ...
                for j in stride(from: _xBounds.min, through: _xBounds.range + _xBounds.min, by: 1)
                {
                    guard let e = dataSet.entryForIndex(j) as? CHCandleChartDataEntry else { break }
    ...
    open override func drawHighlighted(context: CGContext, indices: [CHHighlight])
    {
        ...
        {
            guard
                let set = candleData.getDataSetByIndex(high.dataSetIndex) as? CHICandleChartDataSet,
                set.isHighlightEnabled
                else { continue }
            
            guard let e = set.entryForXValue(high.x, closestToY: high.y) as? CHCandleChartDataEntry else { continue }
    ...
    private func createAccessibleElement(withIndex idx: Int,
                                         container: CHCandleChartDataProvider,
                                         dataSet: CHICandleChartDataSet,
