...
open class CHXAxisRenderer: CHAxisRendererBase
{
    @objc public init(viewPortHandler: CHViewPortHandler, xAxis: CHXAxis?, transformer: CHTransformer?)
    {
    ...
    {
        guard let xAxis = self.axis as? CHXAxis else { return }
    ...
    {
        guard
            let xAxis = self.axis as? CHXAxis,
    ...
    @objc open func renderLimitLineLine(context: CGContext, limitLine: CHChartLimitLine, position: CGPoint)
    {
    ...
    @objc open func renderLimitLineLabel(context: CGContext, limitLine: CHChartLimitLine, position: CGPoint, yOffset: CGFloat)
    {
        ...
        if limitLine.drawLabelEnabled && label.count > 0
        {
            ...
            {
                CHChartUtils.drawText(context: context,
                    text: label,
                    point: CGPoint(
