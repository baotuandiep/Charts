...
open class CHLineChartRenderer: CHLineRadarRenderer
{
    ...
    @objc open weak var dataProvider: CHLineChartDataProvider?
    
    @objc public init(dataProvider: CHLineChartDataProvider, animator: CHAnimator, viewPortHandler: CHViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        for i in 0 ..< lineData.dataSetCount
        {
            ...
            if set.isVisible
            {
                if !(set is CHILineChartDataSet)
                {
                    fatalError("Datasets for LineChartRenderer must conform to ILineChartDataSet")
                }
                
                drawDataSet(context: context, dataSet: set as! CHILineChartDataSet)
            }
        }
    }
    
    @objc open func drawDataSet(context: CGContext, dataSet: CHILineChartDataSet)
    {
    ...
    @objc open func drawCubicBezier(context: CGContext, dataSet: CHILineChartDataSet)
    {
        ...
        if _xBounds.range >= 1
        {
            ...
            var prevPrev: CHChartDataEntry! = nil
            var prev: CHChartDataEntry! = dataSet.entryForIndex(max(firstIndex - 2, 0))
            var cur: CHChartDataEntry! = dataSet.entryForIndex(max(firstIndex - 1, 0))
            var next: CHChartDataEntry! = cur
            var nextIndex: Int = -1
    ...
    @objc open func drawHorizontalBezier(context: CGContext, dataSet: CHILineChartDataSet)
    {
        guard let dataProvider = dataProvider else { return }
        ...
        if _xBounds.range >= 1
        {
            var prev: CHChartDataEntry! = dataSet.entryForIndex(_xBounds.min)
            var cur: CHChartDataEntry! = prev
    ...
    @objc open func drawLinear(context: CGContext, dataSet: CHILineChartDataSet)
    {
        guard let dataProvider = dataProvider else { return }
        ...
        if dataSet.colors.count > 1
        {
            ...
            {
                var e: CHChartDataEntry! = dataSet.entryForIndex(j)
        ...
        { // only one color per dataset
            ...
            var e1: CHChartDataEntry!
            var e2: CHChartDataEntry!
    ...
    open func drawLinearFill(context: CGContext, dataSet: CHILineChartDataSet, trans: CHTransformer, bounds: XBounds)
    {
        guard let dataProvider = dataProvider else { return }
    ...
    private func generateFilledPath(dataSet: CHILineChartDataSet, fillMin: CGFloat, bounds: XBounds, matrix: CGAffineTransform) -> CGPath
    {
        let phaseY = animator.phaseY
        let isDrawSteppedEnabled = dataSet.mode == .stepped
        let matrix = matrix
        
        var e: CHChartDataEntry!
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        if isDrawingValuesAllowed(dataProvider: dataProvider)
        {
            ...
            {
                guard let dataSet = dataSets[i] as? CHILineChartDataSet else { continue }
                ...
                for j in stride(from: _xBounds.min, through: min(_xBounds.min + _xBounds.range, _xBounds.max), by: 1)
                {
                    ...
                    {
                        CHChartUtils.drawImage(context: context,
                                             image: icon,
    ...
    private func drawCircles(context: CGContext)
    {
        ...
        if let chart = dataProvider as? CHLineChartView {
        ...
        for i in 0 ..< dataSets.count
        {
            guard let dataSet = lineData.getDataSetByIndex(i) as? CHILineChartDataSet else { continue }
    ...
    open override func drawHighlighted(context: CGContext, indices: [CHHighlight])
    {
        ...
        for high in indices
        {
            guard let set = lineData.getDataSetByIndex(high.dataSetIndex) as? CHILineChartDataSet
    ...
    {
        guard let chart = dataProvider as? CHLineChartView else { return [] }
