...
open class CHRadarChartRenderer: CHLineRadarRenderer
{
    ...
    @objc open weak var chart: CHRadarChartView?

    @objc public init(chart: CHRadarChartView, animator: CHAnimator, viewPortHandler: CHViewPortHandler)
    {
    ...
    open override func drawData(context: CGContext)
    {
        ...
        if radarData != nil
        {
            ...
            if let accessibilityHeaderData = radarData as? CHRadarChartData {
            ...
            for set in radarData!.dataSets as! [CHIRadarChartDataSet]
            {
                if set.isVisible
                {
    ...
    internal func drawDataSet(context: CGContext, dataSet: CHIRadarChartDataSet, mostEntries: Int)
    {
        guard let chart = chart else { return }
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        {
            let dataSet = data.getDataSetByIndex(i) as! CHIRadarChartDataSet
    ...
    open override func drawHighlighted(context: CGContext, indices: [CHHighlight])
    {
        guard
            let chart = chart,
            let radarData = chart.data as? CHRadarChartData
            else { return }
        ...
        {
            guard
                let set = chart.data?.getDataSetByIndex(high.dataSetIndex) as? CHIRadarChartDataSet,
                set.isHighlightEnabled
                else { continue }
            
            guard let e = set.entryForIndex(Int(high.x)) as? CHRadarChartDataEntry
                else { continue }
    ...
    private func createAccessibleElement(withDescription description: String,
                                         container: CHRadarChartView,
                                         dataSet: CHIRadarChartDataSet,
