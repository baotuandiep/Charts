...
open class CHRadarHighlighter: CHPieRadarHighlighter
{
    open override func closestHighlight(index: Int, x: CGFloat, y: CGFloat) -> CHHighlight?
    {
        guard let chart = self.chart as? CHRadarChartView else { return nil }
    ...
    ///
    /// - parameter index:
    internal func getHighlights(forIndex index: Int) -> [CHHighlight]
    {
        var vals = [CHHighlight]()
        
        guard
            let chart = self.chart as? CHRadarChartView,
        ...
        for i in chartData.dataSets.indices
        {
            ...
            let highlight = CHHighlight(x: Double(index), y: entry.y, xPx: p.x, yPx: p.y, dataSetIndex: i, axis: dataSet.axisDependency)
