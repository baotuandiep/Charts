...
open class CHChartHighlighter : NSObject, CHIHighlighter
{
    /// instance of the data-provider
    @objc open weak var chart: CHChartDataProvider?
    
    @objc public init(chart: CHChartDataProvider)
    {
        self.chart = chart
    }
    
    open func getHighlight(x: CGFloat, y: CGFloat) -> CHHighlight?
    {
    ...
    {
        guard let chart = self.chart as? CHBarLineScatterCandleBubbleChartDataProvider else { return .zero }
    ...
    /// - parameter y:
    /// - returns:
    @objc open func getHighlight(xValue xVal: Double, x: CGFloat, y: CGFloat) -> CHHighlight?
    {
        ...
        let axis: CHYAxis.AxisDependency = leftAxisMinDist < rightAxisMinDist ? .left : .right
    ...
    /// - returns:
    @objc open func getHighlights(xValue: Double, x: CGFloat, y: CGFloat) -> [CHHighlight]
    {
    ...
    internal func buildHighlights(
        dataSet set: CHIChartDataSet,
        dataSetIndex: Int,
        xValue: Double,
        rounding: ChartDataSetRounding) -> [CHHighlight]
    {
        var highlights = [CHHighlight]()
        
        guard let chart = self.chart as? CHBarLineScatterCandleBubbleChartDataProvider else { return highlights }
        ...
        for e in entries
        {
            ...
            let highlight = CHHighlight(x: e.x, y: e.y, xPx: px.x, yPx: px.y, dataSetIndex: dataSetIndex, axis: set.axisDependency)
    ...
    internal func closestSelectionDetailByPixel(
        closestValues: [CHHighlight],
        x: CGFloat,
        y: CGFloat,
        axis: CHYAxis.AxisDependency?,
        minSelectionDistance: CGFloat) -> CHHighlight?
    {
        var distance = minSelectionDistance
        var closest: CHHighlight?
    ...
    internal func getMinimumDistance(
        closestValues: [CHHighlight],
        y: CGFloat,
        axis: CHYAxis.AxisDependency) -> CGFloat
    {
    ...
    internal func getHighlightPos(high: CHHighlight) -> CGFloat
    {
        return high.yPx
    }
