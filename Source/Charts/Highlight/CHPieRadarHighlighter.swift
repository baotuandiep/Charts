...
open class CHPieRadarHighlighter: CHChartHighlighter
{    
    open override func getHighlight(x: CGFloat, y: CGFloat) -> CHHighlight?
    {
        guard let chart = self.chart as? CHPieRadarChartViewBase else { return nil }
    ...
    /// - parameter x:
    /// - parameter y:
    @objc open func closestHighlight(index: Int, x: CGFloat, y: CGFloat) -> CHHighlight?
    {
