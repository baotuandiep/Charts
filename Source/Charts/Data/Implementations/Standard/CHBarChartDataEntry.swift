...
open class BarChartDataEntry: CHChartDataEntry
{
    ...
    @objc open func calcRanges()
    {
        ...
        for i in 0 ..< values!.count
        {
            ...
            if value < 0
            {
                _ranges?.append(CHRange(from: negRemain, to: negRemain - value))
                negRemain -= value
            }
            else
            {
                _ranges?.append(CHRange(from: posRemain, to: posRemain + value))
                posRemain += value
            }
        }
    }
