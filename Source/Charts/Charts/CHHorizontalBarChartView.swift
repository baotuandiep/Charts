...
open class CHHorizontalBarChartView: CHBarChartView
{
    internal override func initialize()
    {
        super.initialize()
        
        _leftAxisTransformer = CHTransformerHorizontalBarChart(viewPortHandler: _viewPortHandler)
        _rightAxisTransformer = CHTransformerHorizontalBarChart(viewPortHandler: _viewPortHandler)
        
        renderer = CHHorizontalBarChartRenderer(dataProvider: self, animator: _animator, viewPortHandler: _viewPortHandler)
        leftYAxisRenderer = CHYAxisRendererHorizontalBarChart(viewPortHandler: _viewPortHandler, yAxis: leftAxis, transformer: _leftAxisTransformer)
        rightYAxisRenderer = CHYAxisRendererHorizontalBarChart(viewPortHandler: _viewPortHandler, yAxis: rightAxis, transformer: _rightAxisTransformer)
        xAxisRenderer = CHXAxisRendererHorizontalBarChart(viewPortHandler: _viewPortHandler, xAxis: _xAxis, transformer: _leftAxisTransformer, chart: self)
        
        self.highlighter = CHHorizontalBarHighlighter(chart: self)
    }
    ...
    open override func getMarkerPosition(highlight: CHHighlight) -> CGPoint
    {
    ...
    open override func getBarBounds(entry e: CHBarChartDataEntry) -> CGRect
    {
        guard
            ...
            let set = data.getDataSetForEntry(e) as? CHIBarChartDataSet
    ...
    open override func getPosition(entry e: CHChartDataEntry, axis: CHYAxis.AxisDependency) -> CGPoint
    {
    ...
    open override func setVisibleYRangeMaximum(_ maxYRange: Double, axis: CHYAxis.AxisDependency)
    {
        let yScale = getAxisRange(axis: axis) / maxYRange
        viewPortHandler.setMinimumScaleX(CGFloat(yScale))
    }
    
    open override func setVisibleYRangeMinimum(_ minYRange: Double, axis: CHYAxis.AxisDependency)
    {
        let yScale = getAxisRange(axis: axis) / minYRange
        viewPortHandler.setMaximumScaleX(CGFloat(yScale))
    }
    
    open override func setVisibleYRange(minYRange: Double, maxYRange: Double, axis: CHYAxis.AxisDependency)
    {
