...
open class CHMarkerView: NSUIView, CHIMarker
{
    open var offset: CGPoint = CGPoint()
    
    @objc open weak var chartView: CHChartViewBase?
    ...
    open func refreshContent(entry: CHChartDataEntry, highlight: CHHighlight)
    {
        // Do nothing here...
    }
    ...
    @objc
    open class func viewFromXib(in bundle: Bundle = .main) -> CHMarkerView?
    {
        #if !os(OSX)
            return bundle.loadNibNamed(
                String(describing: self),
                owner: nil,
                options: nil)?[0] as? CHMarkerView
        #else
